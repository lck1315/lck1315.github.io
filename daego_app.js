...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged((user) => {
        ...
        if (user) {
            ...
            db.collection('daego_users').doc(user.uid).get()
                .then((doc) => {
                    if (doc.exists) {
                    ...
                    userGoogleCalendarListener = db.collection('daego_users').doc(user.uid).onSnapshot((uDoc) => {
    ...
    const savedTheme = localStorage.getItem('daego-theme') || 'dark-theme';
    body.className = savedTheme;

    themeToggleBtn.addEventListener('click', () => {
        if (body.classList.contains('dark-theme')) {
            body.classList.replace('dark-theme', 'light-theme');
            localStorage.setItem('daego-theme', 'light-theme');
        } else {
            body.classList.replace('light-theme', 'dark-theme');
            localStorage.setItem('daego-theme', 'dark-theme');
        }
        initParticles(); 
    });
    ...
    function initEventsListener() {
        db.collection('daego_events').onSnapshot((snapshot) => {
            familyEvents = {};
    ...
    eventForm.addEventListener('submit', async (e) => {
        ...
        try {
            for (let dStr of datesToSave) {
                ...
                await db.collection('daego_events').doc(dStr).set({
                    events: newEvents
                });
            }
    ...
    function deleteEvent(index) {
        ...
        if (confirm('이 일정을 삭제하시겠습니까?')) {
            ...
            if (currentEvents.length === 0) {
                db.collection('daego_events').doc(activeSelectedDateStr).delete()
                    .catch(err => console.error("Error deleting events: ", err));
            } else {
                db.collection('daego_events').doc(activeSelectedDateStr).set({
    ...
    if (galleryForm) {
        galleryForm.addEventListener('submit', (e) => {
            ...
            if (editId) {
                // 수정
                db.collection('daego_gallery_posts').doc(editId).update(newAlbum).then(async () => {
                    try {
                        // 기존 이미지들 일괄 삭제
                        const oldImagesSnapshot = await db.collection('daego_gallery_images').where('postId', '==', editId).get();
                        ...
                        for (let i = 0; i < compressedGalleryImages.length; i++) {
                            await db.collection('daego_gallery_images').add({
                                postId: editId,
                ...
                // 신규 등록
                db.collection('daego_gallery_posts').add(newAlbum).then(async (docRef) => {
    ...
    function startEditGalleryPost(id) {
        ...
        db.collection('daego_gallery_posts').doc(id).get()
            .then(async doc => {
                if (doc.exists) {
                    const post = doc.data();
                    ...
                    try {
                        const imagesSnapshot = await db.collection('daego_gallery_images').where('postId', '==', id).orderBy('index', 'asc').get();
                        let postImages = [];
    ...
    function initGalleryListener() {
        db.collection('daego_gallery_posts').orderBy('date', 'desc').onSnapshot(snapshot => {
            const dbPosts = [];
            snapshot.forEach(doc => {
        ...
        db.collection('daego_gallery_images').orderBy('date', 'desc').onSnapshot(snapshot => {
            const dbImages = [];
            snapshot.forEach(doc => {
                ...
                if (delBtn) {
                    delBtn.addEventListener('click', (ev) => {
                        ...
                        if (confirm("이 앨범을 삭제하시겠습니까?")) {
                            db.collection('daego_gallery_posts').doc(post.id).delete()
                                .then(async () => {
                                    try {
                                        const oldImagesSnapshot = await db.collection('daego_gallery_images').where('postId', '==', post.id).get();
                                        const deletePromises = [];
                                        oldImagesSnapshot.forEach(doc => deletePromises.push(doc.ref.delete()));
                                        await Promise.all(deletePromises);
                                    } catch (err) {
    ...
    function initBoardListener() {
        db.collection('daego_board_posts').orderBy('date', 'desc').onSnapshot((snapshot) => {
            boardPosts = [];
            snapshot.forEach((doc) => {
                boardPosts.push({
                    id: doc.id,
...
function initCardSliders(container) {
    ...
    function startEditBoardPost(id) {
        ...
        db.collection('daego_board_posts').doc(id).get()
            .then(doc => {
                if (doc.exists) {
                    const post = doc.data();
                    const isOwner = currentUserInfo && (post.uid === currentUserInfo.uid || (!post.uid && post.author === currentUserInfo.nickname));
    ...
    boardForm.addEventListener('submit', (e) => {
        ...
        if (editId) {
            db.collection('daego_board_posts').doc(editId).update({
                title,
                content,
            ...
            db.collection('daego_board_posts').add(newPost).then(() => {
                resetBoardForm();
            }).catch(err => {
    ...
    function deleteBoardPost(id) {
        ...
        if (confirm('이 게시글을 정말로 삭제할까요?')) {
            db.collection('daego_board_posts').doc(id).delete()
    ...
    function initGuestbookListener() {
        db.collection('daego_messages').orderBy('date', 'desc').onSnapshot((snapshot) => {
            messages = [];
            const likedMessages = JSON.parse(localStorage.getItem('daego-liked-messages')) || [];
            snapshot.forEach((doc) => {
                const data = doc.data();
                const msgId = doc.id;
                messages.push({
    ...
    function startEditGuestbookMessage(id) {
        ...
        db.collection('daego_messages').doc(id).get()
            .then(doc => {
                if (doc.exists) {
                    const msg = doc.data();
                    const isOwner = currentUserInfo && (msg.uid === currentUserInfo.uid || (!msg.uid && msg.author === currentUserInfo.nickname));
    ...
    guestbookForm.addEventListener('submit', (e) => {
        ...
        if (editId) {
            db.collection('daego_messages').doc(editId).update({
                message,
            ...
            db.collection('daego_messages').add(newMessage).then(() => {
                resetGuestbookForm();
            }).catch(err => {
    ...
    function toggleLike(id) {
        ...
        const likedMessages = JSON.parse(localStorage.getItem('daego-liked-messages')) || [];
        const msgRef = db.collection('daego_messages').doc(id);
        ...
        db.runTransaction((transaction) => {
            ...
            localStorage.setItem('daego-liked-messages', JSON.stringify(newLikedList));
        }).catch((err) => {
    ...
    function deleteMessage(id) {
        ...
        if (confirm('이 방명록 글을 정말 삭제할까요?')) {
            db.collection('daego_messages').doc(id).delete()
    ...
    window.toggleApproval = function(userId, approve) {
        ...
        if (confirm(`이 회원을 ${actionStr} 처리하시겠습니까?`)) {
            db.collection('daego_users').doc(userId).update({ isApproved: approve })
                .then(() => {
    ...
    window.rejectUser = function(userId) {
        if (confirm("이 회원을 정말 데이터베이스에서 삭제하시겠습니까? (복구 불가능하며 접속이 영구 차단됩니다.)")) {
            db.collection('daego_users').doc(userId).delete()
                .then(() => {
    ...
    if (adminEditUserForm) {
        adminEditUserForm.addEventListener('submit', (e) => {
            ...
            db.collection('daego_users').doc(uid).update({
                nickname: newName,
                role: newRole
            }).then(() => {
    ...
    let heroImages = ["./assets/daego_hero.png"]; // 메모리 내 이미지 목록
    let heroSliderIndex = 0;
    let heroSliderInterval = null;

    function initSettingsListener() {
        // 메인 이미지 목록을 settings/main_image 문서 내 images 배열에서 실시간 수신
        db.collection('daego_settings').doc('main_image').onSnapshot((doc) => {
            if (doc.exists && doc.data().images && doc.data().images.length > 0) {
                heroImages = doc.data().images;
            } else {
                heroImages = ["./assets/daego_hero.png"];
            }
            // 캐시 최신화
            localStorage.setItem('daego-hero-images-cache', JSON.stringify(heroImages));
        ...
        checkboxes.forEach(chk => {
            chk.addEventListener('change', (e) => {
                ...
                if (googleCalendarUrls[idx]) {
                    ...
                    // Firestore에 실시간 저장
                    if (currentUserInfo && currentUserInfo.uid) {
                        db.collection('daego_users').doc(currentUserInfo.uid).set({
        ...
        heroSliderFileInput.addEventListener('change', (e) => {
            ...
            reader.onload = function(event) {
                ...
                img.onload = function() {
                    ...
                    db.collection('daego_settings').doc('main_image').set({
    ...
    if (indexNoticeContainer) {
        db.collection('daego_familyNotices').orderBy('createdAt', 'desc').onSnapshot((snapshot) => {
    ...
    if (btnSubmitNotice) {
        btnSubmitNotice.addEventListener('click', () => {
            ...
            db.collection('daego_familyNotices').add({
                title: title,
                content: content,
